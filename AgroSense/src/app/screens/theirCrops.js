...
// Asegúrate de inicializar Firebase en tu proyecto.
...
const CultivosScreen = () => {
  ...
  const [selectedEtapa, setSelectedEtapa] = useState(""); // Selector para la etapa de crecimiento
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [weeklyImages, setWeeklyImages] = useState([]);
  const [cultivos, setCultivos] = useState([]);

  // Opciones de etapas de crecimiento con semanas
  ...
  const agregarCultivo = async () => {
    const nuevoCultivo = {
      ...
      nombre: nameCultivo,
      etapa: selectedEtapa, // Almacena la etapa seleccionada
    ...
    try {
      ...
      setCultivos([...cultivos, { ...nuevoCultivo, id: cultivos.length + 1 }]);
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList data={cultivos} renderItem={renderCultivo} keyExtractor={(item) => item.id.toString()} />
